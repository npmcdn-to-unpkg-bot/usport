    ...
    */
    ...
    public function commentPost (Request $request) {
        ...
        if (isset($result->id) && strlen($result->id) > 0) {
            if ($postId == 0) {
                $postData = $this->post->getPostBy('fb_post_id', $fbPostId);//->id;
                if ($postData) {
                    $postId = $postData->id;
                }
            }
            ...
            if ($comment->save()) {
                try {
                    ...
                    return response()->json(array('error'=>1003, 'data'=>'', 'message'=>'cannot comment'));
                }
            }
        }
     ...
     * @return [type]          [description]
     */
    public function getCommentsByPostId($postId, $offset=0, $limit=10, $order='id', $by='DESC') {
    ...
    protected function listPost($offset, $limit, $order, $by, $user) {
        ...
        foreach ($posts as $p) {
            ...
            $comments = PostComment::where('post_id', $p->id)->orderBy('id', 'DESC')->limit(config('cc.comments.number_show'))->get(array('id', 'content', 'user_id', 'display_name', 'avatar', 'created_at', 'updated_at'));
            $p->comments = array('offset'=>config('cc.comments.offset'), 'data'=>$comments);
        }
