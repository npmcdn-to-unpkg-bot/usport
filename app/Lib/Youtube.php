...
class Youtube {
	...
	public function getLatestAccessTokenFromDB()
	{
		...
		// \Log::info('latest: ' . json_encode($latest));
		if ($latest) {
			...
			\Log::info('expired: ' . strtotime($latest->created_at) . ':' . ($latest->expires_in - 30) . ':' . $currentTime . ':' . ((strtotime($latest->created_at) + ($latest->expires_in - 30)) - $currentTime));
			return array(
