    ...
    $scope.shareFb = function (id) {
        ...
        $timeout(function () {
            $window.FB.ui({method: "share", href: url}, function (response) {
                ...
                // Object { post_id="157828207958185"}
                if (typeof response.post_id != 'undefined') {
                    var params = {fb_post_id:response.post_id, post_id:id};
                    console.log(params);
    ...
    $scope.$on("onListModifiedConversationReceived", function (event, packet) {
        if (packet.done == 1) {
            ...
            for(var i in packet.list) {
                packet.list[i].avatar = '/images/avatar.png';
...
.controller('ComposeCtrl', ['$scope','$http','$location','$rootScope', '$uibModal', '$uibModalInstance', '$state', function($scope,$http,$location,$rootScope, $uibModal, $uibModalInstance, $state) {
    ...
    $scope.save = function(){
        ...
        if(params.attachments == ''){
            $scope.error = 1;
            $scope.error_msg = 'Vui lòng đính kèm video';
            console.log(params)
            return;
        }
        if(params.content == ''){
            $scope.error = 1;
            $scope.error_msg = 'Vui lòng nhập giới thiệu về video của bạn';
            console.log(params)
            return;
        }
...
.controller('LoginCtrl', ['$scope','$http','$location','headerFooterData','$uibModalInstance','$uibModal','AuthService', '$rootScope', '$localStorage', function($scope, $http, $location, headerFooterData, $uibModalInstance, $uibModal, AuthService, $rootScope, $localStorage) {
    ...
    $scope.loginFacebook = function (token) {
        FB.login(function(response) {
            ...
            scope: 'user_birthday,user_friends,email,public_profile,user_likes,user_posts,publish_actions', 
            return_scopes: true
        });
    };
    $scope.closeModal = function () {
